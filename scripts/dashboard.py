...
"""
ACTDIM-AGENTS Repository Dashboard & Analytics Engine.
...
- Markdown report mode: Updates .agents/DASHBOARD.md with Mermaid diagrams.
"""
...
def find_agents_dir(start_dir: str = ".") -> Path:
    """Find nearest .agents directory starting from start_dir and walking upwards."""
    ...
    return Path(start_dir).resolve() / ".agents"
...
def render_cli(collector: AgentEntityCollector):
    ...
    if HAS_RICH:
        ...
        console.print()
        console.print(Panel(
            f"[bold cyan]ACTDIM-AGENTS Repository Dashboard[/bold cyan] [dim]({collector.repo_root.name})[/dim]\n"
        ...
        print("=" * 70)
        print(f"ACTDIM-AGENTS Repository Dashboard ({collector.repo_root.name})")
...
# ----------------------------------------------------------------------
# 3. Markdown Report Generator (.agents/DASHBOARD.md)
# ----------------------------------------------------------------------

def generate_markdown_report(collector: AgentEntityCollector, output_path: Path):
    m = collector.metrics
    lines = []
    lines.append("# ACTDIM-AGENTS Executive Dashboard & Repository Analytics")
    lines.append("")
    ...
    lines.append(f"| **Knowledge Base** | `{m['total_kb_articles']}` articles | `.agents/KB/` documentation |")
    lines.append(f"| **Context Footprint** | `{m['context_lines']}` lines | `.agents/CONTEXT.md` |")
    lines.append("")
...
<head>
  ...
  <title>ACTDIM-AGENTS Dashboard - {{ repo_name }}</title>
...
def run_web_server(collector: AgentEntityCollector, host: str = "127.0.0.1", port: int = 8765, open_browser: bool = True):
    ...
    app = FastAPI(title="ACTDIM-AGENTS Dashboard")
    ...
    print(f"-> Starting ACTDIM-AGENTS FastAPI Dashboard at {url}")
...
def main():
    parser = argparse.ArgumentParser(description="ACTDIM-AGENTS Repository Dashboard & Analytics")
    ...
    parser.add_argument("-m", "--markdown", action="store_true", help="Generate or update .agents/DASHBOARD.md")
    parser.add_argument("-e", "--export", nargs="?", const=".agents/dashboard.html", help="Export standalone static HTML dashboard (default: .agents/dashboard.html)")

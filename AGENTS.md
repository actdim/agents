...
- **Skills Source**: `skills/` (`init-agents`, `update-agents`, `init-kb`, `search-kb`, `sync-kb`, `check-graph`, `sync-history`, `bump-version`, `wrap-session`, `sync-context`, `sync-issues`, `sync-decisions`, `sync-tasks`).
- **Install Commands**:

...
## Scope
`actdim-agents` is a provider-agnostic, zero-friction agent memory and project management protocol and skills suite for AI coding assistants (Claude Code, OpenAI Codex, OpenCode, Google Antigravity). It equips repositories with:
1. **Persistent Project Memory**: Structured `.agents/` repository memory across sessions and tools.
2. **Automated Entity Lifecycle**: Zero-friction tracking of issues, milestones, risks, spikes, and checklists in the background.
3. **Structured Knowledge Base (KB)**: Scalable, fact-grounded documentation (`.agents/KB/`) with hybrid search capabilities.
4. **Visual Dashboard & Analytics**: Executive project dashboard (`/repo-dashboard`) visualizing velocity, milestone progress, and repository health.
...
- [x] **Phase 2 (Knowledge Base & Code Graph)**: `.agents/KB/`, `/init-kb`, `/search-kb`, `/sync-kb`, `/check-graph`, and `code-review-graph` MCP integration.
...
- [ ] **Phase 4 (Visual Dashboard & Repository Analytics)**: `/repo-dashboard` skill, Generative UI widgets, Mermaid progress charts, and CLI executive summaries.

...
Completed major protocol architectural upgrade to v1.5.0, established the versioned migration engine with retroactive entity synthesis, introduced Language Rule Packs, and built the `/sync-history` Git reconciliation skill.

## Changes Made

1. **Protocol v1.5.0 & Entity Ecosystem**:
   - Upgraded `AGENTS.md` and `skills/init-agents/protocol.md` with structured YAML front-matter schemas for `MILESTONES`, `RISKS`, `SPIKES`, `CHECKLISTS`, `SESSIONS`, `KB`, and `ISSUES`.
...
4. **Git Reconciliation Skill (`/sync-history`)**:
   - Created `skills/sync-history/SKILL.md` and `skills/sync-history/scripts/analyze_git_history.py`.
...
5. **Project Memory & Versioning**:
   ...
   - Synchronized version `1.5.0` across 21 files via `scripts/bump-version.py`.

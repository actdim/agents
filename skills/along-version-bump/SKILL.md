...
description: Increment project or protocol version (patch by default), sanitize typography, execute wrap-session reconciliation, and create a release git commit. Universal support for Node, Python, Rust, .NET, and custom .along/scripts/bump_version.py hooks. Use when invoking /along-version-bump (aliases: /version-bump, /along-bump-version).
---

# Along Version Bump (`/along-version-bump`, `/version-bump`) [v2.1.1]
...
- Command: `/along-version-bump` (aliases: `/version-bump`, `/along-bump-version`)

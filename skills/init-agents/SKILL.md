...
- **Strict File Modification & Anti-Deletion**:
  ...
  - **Minimal Edit Scope**: Anchor edit blocks strictly on exact single lines or minimal unique chunks. Avoid replacing large multi-line surrounding blocks.
  - **Mandatory Diff Verification**: After modifying any file, inspect the generated diff (or run `git diff <file>`) to ensure only intended lines were added/modified and no adjacent content was accidentally removed.
  - **Immediate Rollback**: If an unintended deletion or truncation is detected in the diff, restore the missing lines immediately before proceeding.
